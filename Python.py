...
# ==============================================================================
BOT_TOKEN = "8692409769:AAFkwguQsa5n06-BFic3h7ABPCVki2Fz3mU"
BOT_USERNAME = "ISHAN_PANELS_SELLING_BOT"
ADMIN_ID = 8666283123
ADMIN_CONTACT = "ISHAN_MODS_XYT"

# Always use the database beside this Python.py file, regardless of the
# directory from which Termux/tmux starts the bot.
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Cuibcc.db")
...
WELCOME_STICKER_ID = "CAACAgUAAxkBAAEGQxlqho8Ge0YpBwL6wpUxsZvtGJaqwQACgg8AAsqhMFb71jlzW2MGFD0E"  # Replace with your sticker ID
...
DEFAULT_EMOJIS = {
    ...
    'profile': '5870695289714643076',
    'add_balance': '5429651785352501917',
    'history': '6032594876506312598',
    'referral': '5350452584119279096',
    'support': '5472239203590888751',
    'ludo_spin': '5382194935057372936',
    'back': '5409357944619802453',
    'upi': '5807750375033278838',
    'binance': '6161089551994462716',
    'reseller': '5447410659077661506',
    'tutorial': '5427158904729513162',
    'download': '6161336001512874965',
    'telegram': '5456140674028019486',
    'whatsapp': '5251203410396458957',
    'welcome': '5397782960512444700',
...
UI_TEXTS = {
    "start_menu": (
        "✨ <b>𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗧𝗛𝗘 𝗜𝗦𝗛𝗔𝗡 𝗣𝗔𝗡𝗘𝗟𝗦 𝗦𝗛𝗢𝗣</b>\n\n"
...
def admin_kb() -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="⚙️ FamPay Setup", callback_data="admin_setup_fampay", style="success"),
...
# ==============================================================================
# 9. FAMPAY PAYMENT FUNCTIONS - FIXED WITH QR IMAGE DIRECTLY IN BOT
# ==============================================================================
...
async def generate_fampay_qr(user_id: int, amount: float, upi_id: str = None) -> Dict[str, Any]:
    ...
    order_id = f"FAMPAY{user_id}{int(time.time())}{random.randint(100, 999)}"
    
    url = f"{FAMPAY_QR_URL}?upi={upi_id}&amount={amount}&order_id={order_id}"
            ...
            async with session.get(url) as resp:
                if resp.status == 200:
                    try:
                        ...
                        return {"status": "error", "message": "Failed to parse response from payment gateway"}
                else:
                    return {"status": "error", "message": f"Gateway HTTP Error: {resp.status}"}
        except asyncio.TimeoutError:
            return {"status": "error", "message": "Gateway connection timeout. Please try again."}
        except Exception as e:
        ...
        try:
            async with session.get(url) as resp:
                if resp.status == 200:
                    ...
                    return {"status": "error", "message": f"Verification HTTP Error: {resp.status}"}
        except asyncio.TimeoutError:
            return {"status": "error", "message": "Verification timeout. Please try again."}
        except Exception as e:
...
async def run_payment_verification(user_id: int, order_id: str, reply_target: Any) -> None:
    ...
    if not txn:
        ...
        if isinstance(reply_target, CallbackQuery): 
            ...
            await reply_target.edit_text(err, reply_markup=back_kb(), parse_mode='HTML')
        return
    ...
    if txn[5] and time.time() > txn[5]:
        ...
        if isinstance(reply_target, CallbackQuery): 
            ...
            await reply_target.edit_text(err_msg, reply_markup=back_kb(), parse_mode='HTML')
        return
    ...
    if txn[1] == 'paid':
        ...
        if isinstance(reply_target, CallbackQuery): 
            ...
            await reply_target.edit_text(msg, reply_markup=back_kb(), parse_mode='HTML')
        return
    elif txn[1] == 'expired':
    ...
    if isinstance(reply_target, CallbackQuery):
        await reply_target.message.edit_text("🔄 <b>Verifying payment...</b>\n<i>Please wait while we confirm your transaction.</i>", parse_mode='HTML')
    
    result = await verify_fampay_payment(order_id)
    
    if result.get("status") == "success":
        txn_data = result.get("data", {})
        transaction_id = txn_data.get("transaction_id", order_id)
        utr = txn_data.get("utr", "N/A")
        sender_name = txn_data.get("sender_name", "Unknown")
        amount_received = txn_data.get("amount", txn[0])
        payment_time = txn_data.get("payment_time_ist", datetime.now().strftime("%d-%m-%Y %H:%M:%S"))
        ...
        success_msg = (
            ...
            f"💰 <b>New Balance:</b> {fmt_curr(txn[0] + amount_received)}"
        )
        if isinstance(reply_target, CallbackQuery): 
            await reply_target.message.edit_text(success_msg, reply_markup=back_kb(), parse_mode='HTML')
        else: 
            await reply_target.edit_text(success_msg, reply_markup=back_kb(), parse_mode='HTML')
        
        await send_advanced_notification(user_id, "DEPOSIT", amount_received, product=order_id, gateway="FamPay")
        ...
        pending_msg = (
            ...
            f"{result.get('message', 'We are waiting for payment confirmation.')}\n\n"
            f"💡 <b>Tips:</b>\n"
            f"• Make sure you sent the exact amount\n"
            f"• Wait 10-15 seconds after payment\n"
            f"• Click Verify again if needed\n\n"
            f"<i>Auto-verification will detect payment in 30 seconds.</i>"
        )
        if isinstance(reply_target, CallbackQuery):
            await reply_target.message.edit_text(pending_msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Retry Verification", callback_data=f"verify_{order_id}", style="primary")],
                [InlineKeyboardButton(text="Cancel", callback_data="menu_add_balance", icon_custom_emoji_id=get_emoji_icon("back"), style="danger")]
            ]), parse_mode='HTML')
        else:
            await reply_target.edit_text(pending_msg, reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔄 Retry Verification", callback_data=f"verify_{order_id}", style="primary")],
                [InlineKeyboardButton(text="Cancel", callback_data="menu_add_balance", icon_custom_emoji_id=get_emoji_icon("back"), style="danger")]
            ]), parse_mode='HTML')
        ...
        fail_msg = (
            ...
            f"{result.get('message', 'Transaction could not be completed.')}\n\n"
            f"<i>Please check your payment and try again.</i>"
        )
        if isinstance(reply_target, CallbackQuery):
            await reply_target.message.edit_text(fail_msg, reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
        else:
            await reply_target.edit_text(fail_msg, reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
    else:
        err = f"⚠️ <b>Gateway Error</b>\n\n{result.get('message', 'Unknown error occurred.')}"
        if isinstance(reply_target, CallbackQuery):
            await reply_target.message.edit_text(err, reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
        else:
            await reply_target.edit_text(err, reply_markup=back_kb("gateway_inr"), parse_mode='HTML')
            ...
            if result.get("status") == "success":
                ...
                try:
                    await bot.send_message(
                        user_id, 
                        f"✨ <b>AUTO-VERIFIED!</b>\n\n"
                        f"✅ Your payment of {fmt_curr(amount_received)} was detected successfully!\n"
                        f"🧾 <b>UTR:</b> <code>{utr}</code>\n"
                        f"👤 <b>Sender:</b> {sender_name}\n"
                        f"📅 <b>Time:</b> {payment_time}\n\n"
                        f"💰 <b>New Balance:</b> {fmt_curr(amount_received)}",
                        parse_mode='HTML'
                    )
                except: pass
                ...
                try:
                    await bot.send_message(
                        user_id,
                        f"❌ <b>Payment Failed</b>\n\n"
                        f"Your payment for order <code>{order_id}</code> could not be verified.\n"
                        f"Reason: {result.get('message', 'Unknown error')}\n\n"
                        f"Please contact support if you have already paid.",
                        parse_mode='HTML'
                    )
                except: pass
...
# ==============================================================================
# 12. FAMPAY UPI PAYMENT FLOW - QR IMAGE DIRECTLY IN BOT MESSAGE
# ==============================================================================
...
async def add_balance_inr(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="₹5000", callback_data="pay_5000", style="primary")],
    ...
    if action == "confirm":
        ...
        try:
            ...
            if amount < 10:
                ...
                return
            if amount > 5000:
                await call.answer("Maximum deposit is ₹5000.", show_alert=True)
                return
            await state.clear()
...
async def generate_fampay_order(user_id: int, inr_amount: float, message_obj: Message) -> None:
    """Generate FamPay QR code - QR image directly in bot message."""
    api_key = get_setting("fampay_api_key", "")
    if not api_key or api_key == "YOUR_FAMPAY_API_KEY":
    ...
    order_id = f"FAMPAY{user_id}{current_time}{random.randint(100, 999)}"
    ...
    created_at = data.get("created_at_ist", datetime.now().strftime("%d-%m-%Y %H:%M:%S"))
    ...
    try:
        if expires_at_str:
            ...
            expires_at_str = expiry_time.strftime("%d-%m-%Y %H:%M:%S")
    ...
    # Create caption with payment details
    caption = (
        f"🧾 <b>SECURE QR CODE GENERATED</b>\n\n"
        f"💵 <b>Amount:</b> {fmt_curr(inr_amount)}\n"
        f"🆔 <b>Order ID:</b> <code>{order_id}</code>\n"
        f"🏦 <b>UPI ID:</b> <code>{upi_id}</code>\n"
        f"⏳ <b>Expires:</b> {expires_at_str}\n"
        f"📅 <b>Created:</b> {created_at}\n\n"
        f"📱 <b>How to Pay:</b>\n"
        f"1️⃣ Scan the QR using any UPI app\n"
        f"2️⃣ Pay the exact amount: {fmt_curr(inr_amount)}\n"
        f"3️⃣ Click <b>Verify Payment</b> after sending\n"
        f"4️⃣ Auto-verify will also detect payment!\n\n"
        f"<i>Your balance will be credited automatically after successful verification.</i>"
    )
    
    # Create inline keyboard - ONLY Verify and Cancel buttons
    kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    try:
        # Download QR image from URL and send as photo
        async with aiohttp.ClientSession() as session:
            async with session.get(qr_url) as resp:
                if resp.status == 200:
                    ...
                    await message_obj.answer_photo(
                        photo=BufferedInputFile(qr_data, filename="qr.png"),
                        caption=caption,
                        reply_markup=kb,
                        parse_mode='HTML'
                    )
                else:
                    # If QR download fails, show URL
                    await message_obj.edit_text(
                        f"⚠️ <b>QR Code generated but could not be displayed</b>\n\n"
                        f"Please use this link: {qr_url}\n\n{caption}",
                        reply_markup=kb,
                        parse_mode='HTML'
                    )
    except Exception as e:
        logger.error(f"Error sending QR photo: {e}")
        # Fallback - show QR URL
        await message_obj.edit_text(
            f"⚠️ <b>QR Code generated but could not be displayed</b>\n\n"
            f"Please use this link: {qr_url}\n\n{caption}",
            reply_markup=kb,
...
# ==============================================================================
# REST OF THE CODE - Shop, Profile, Orders, Admin, etc. (Same as before)
# ==============================================================================

# ... (All remaining functions from original code - shop, profile, orders, admin panel, etc.)
# I'm keeping them unchanged, just showing the important modified parts above.
...
# ==============================================================================
# 15. USER DASHBOARD, FILES, VIP, RESELLER, ORDERS, PROFILE, REFERRAL
# ==============================================================================
...
async def all_files_handler(call: CallbackQuery):
    ...
    if link:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Access Download Channel", url=link, icon_custom_emoji_id=get_emoji_icon("download"), style="success")],
            [InlineKeyboardButton(text="BACK", callback_data="back_main", icon_custom_emoji_id=get_emoji_icon("back"), style="danger")]
        ])
